...
class DayTrader:
    ...
    def _calculate_position_size(self, confidence: int, available_cash: float) -> float:
        """
        Scale buy amount by Grok confidence and available cash.

        100% confidence → 100% of cash
        75% confidence  → 50% of cash
        50% confidence  → 25% of cash
        <50%            → $0 (skip)

        Formula: buy_amount = cash * (0.25 + 0.75 * (confidence - 50) / 50)
        """
        if confidence < _MIN_CONFIDENCE:
            return 0.0
        conf_ratio = min(1.0, (confidence - 50) / 50)  # 0.0 at 50%, 1.0 at 100%
        allocation = 0.25 + 0.75 * conf_ratio           # 25% at 50%, 100% at 100%
        return min(available_cash, available_cash * allocation)
    ...
    def _run_premarket_scan(self, today: str, now) -> Optional[Dict]:
        ...
        system_prompt = """You are the HighTrade Day Trader — an intraday momentum specialist.
Your ONLY job: find the single best stock to buy at 9:35 AM and sell before close today.

STRATEGY: Catalyst-driven intraday momentum plays.
Find stocks that will move 1-5% TODAY due to a specific catalyst.

WHAT TO LOOK FOR (priority order):
1. Pre-market movers: stocks up 3%+ in pre-market with NEWS catalyst
2. Earnings reactions: stocks gapping on earnings reported last night/this morning
3. Analyst upgrades/downgrades announced this morning
4. FDA decisions, contract wins, product launches happening TODAY
5. Sector momentum: sector ETF moves with clear leadership stock
6. X/social velocity: stocks trending on FinTwit with price confirmation

AVOID:
- Stocks under $5 (illiquid, high spread)
- Stocks with market cap under $1B (manipulation risk)
- Meme stocks without a real catalyst
- Stocks already extended 10%+ pre-market (chasing)
- ADRs with limited US trading volume

EXIT TARGETS — you MUST set these based on the setup:
- stop_loss_pct: How much downside before we cut (0.5% to 2.0%). Tighter for low-vol, wider for volatile.
- take_profit_pct: Primary target (1.5% to 6.0%). Based on expected catalyst magnitude.
- stretch_target_pct: If momentum is strong, how far can it run? (4.0% to 10.0%)

Use web search and X search to find TODAY's catalysts. Check pre-market prices and volume.
...
Respond with ONLY valid JSON:
{
  ...
  "stop_loss_pct": float (e.g. 1.0 for 1%),
  "take_profit_pct": float (e.g. 3.0 for 3%),
  "stretch_target_pct": float (e.g. 6.0 for 6%),
  "risk": "Primary risk for this trade",
  "thesis": "2-3 sentence thesis for why this moves today",
  "sources": ["list of sources/posts found"]
        ...
        user_prompt = (
            ...
            f"Available trading capital: ${available_cash:,.0f}\n\n"
            "Search the web and X right now for:\n"
            "1. Stocks trending in pre-market trading\n"
            ...
            "Give me your #1 pick for a day trade. ONE stock only."
        )
        ...
        position_size = self._calculate_position_size(confidence, available_cash)
        ...
        if status == 'skipped':
            logger.info(f"  ⏭️  Confidence {confidence}% < {_MIN_CONFIDENCE}% — skipping trade today")
            return None
